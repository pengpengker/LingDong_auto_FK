...
class JsApiPay
{
	 ...
	 * @return 用户的openid
	 */
	public function GetOpenid($myurl = null)
	{
		//通过code获得openid
		if (!isset($_GET['code'])){
			//触发微信返回code码
     ...
     * @param string $openid 调用【网页授权获取用户信息】接口获取到用户在该公众号下的Openid
     * @param string $access_token 调用公众号$access_token

     * @return string

     */
    public function getIsGzh($openid,$access_token)
    {
        $response = self::curlGet('https://api.weixin.qq.com/cgi-bin/user/info?access_token='.$access_token.'&openid='.$openid);
        return json_decode($response,true);
    }
    ...
    //获取系统acctoken
    public function getSysWxAcctoken()
    {
    	$SysAccToken = Cache::get('system_wx_acctoken');
    	if($SysAccToken){
    		return $SysAccToken;
    	}else{
    		$config = new WxPayConfig();
	        ...
	        $SysAccToken = $access_token1['access_token'];
	        Cache::set('system_wx_acctoken',$SysAccToken,$access_token1['expires_in']);
	        return $SysAccToken;
    	}
    }

<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:82:"F:\MaLong\item\faka\application\templates\pc\merchant\default\complaint\index.html";i:1539744088;s:71:"F:\MaLong\item\faka\application\templates\pc\merchant\default\base.html";i:1585486427;}*/ ?>
<!DOCTYPE html>
<html lang="zh">
    <head>
        <meta charset="utf-8">
					...
					<form class="form-inline" role="form" action="" method="get">
						<div class="form-group">
							<select name="status" class="form-control">
								...
								<option value="1" <?php if(\think\Request::instance()->get('status')=='1'): ?>selected<?php endif; ?>>已处理</option>
								<option value="2" <?php if(\think\Request::instance()->get('status')=='2'): ?>selected<?php endif; ?>>未处理</option>
								<option value="-1" <?php if(\think\Request::instance()->get('status')=='-1'): ?>selected<?php endif; ?>>已撤销</option>
							</select>
						</div>
						<div class="form-group">
							<select name="type" class="form-control">
								<option value="">全部类型</option>
								<option value="无效卡密" <?php if(\think\Request::instance()->get('type')=='无效卡密'): ?>selected<?php endif; ?>>无效卡密</option>
								<option value="虚假商品" <?php if(\think\Request::instance()->get('type')=='虚假商品'): ?>selected<?php endif; ?>>虚假商品</option>
								<option value="非法商品" <?php if(\think\Request::instance()->get('type')=='非法商品'): ?>selected<?php endif; ?>>非法商品</option>
								<option value="侵权商品" <?php if(\think\Request::instance()->get('type')=='侵权商品'): ?>selected<?php endif; ?>>侵权商品</option>
								<option value="不能购买" <?php if(\think\Request::instance()->get('type')=='不能购买'): ?>selected<?php endif; ?>>不能购买</option>
								<option value="恐怖色情" <?php if(\think\Request::instance()->get('type')=='恐怖色情'): ?>selected<?php endif; ?>>恐怖色情</option>
								<option value="其他投诉" <?php if(\think\Request::instance()->get('type')=='其他投诉'): ?>selected<?php endif; ?>>其他投诉</option>
							</select>
						</div>
				...
				<table class="table table-striped m-0">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<?php if($v['status']==0): ?>处理中<?php elseif($v['status']==1): ?>已处理<?php elseif($v['status'] == -1): ?>已撤销<?php endif; ?>
							...
							<!--<a href="javascript:void(0);" onclick="del(this,<?php echo $v['id']; ?>)">删除</a>-->
								<a href="javascript:void(0);" onclick="detail(this,'<?php echo $v['id']; ?>')">沟通处理</a>
							</td>
						</tr>
						<?php endforeach; ?>
					</tbody>
...
<script>
	...
	function detail(obj,id){
	    layer.open({
            type: 2,
            content: "<?php echo url('Merchant/Complaint/detail'); ?>?id=" + id,
			area:['80%','80%']
		})
	}
